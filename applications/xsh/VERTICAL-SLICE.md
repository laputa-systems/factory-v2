# XSH VS-001: native-host epistemic-to-product vertical slice

[`GLOSSARY.md`](GLOSSARY.md) is canonical for XSH application terms and
spellings in this contract. [`ARCHITECTURE.md`](ARCHITECTURE.md) records the
XSH application architecture; the repository-root documents define the generic
society apparatus. This file owns the first XSH executable proof.
...
delivery attestation are distinct gates. The M5 kernel stores the correlation
and digest but cannot independently observe pipe I/O; the daemon-private
provider-free Office bridge now binds that attestation to `PiSupervisor`'s
complete physical write. The serving loop still has no scheduler call site for
the bridge. Later session commands use the same closed adapter stream and
sequence; neither `AdapterReady` nor `SessionReady` alone authorizes a paid
model turn.
...
The first implementation target spans the generic repository root and this
...
├── Cargo.toml
├── crates/
│   ├── society-kernel/        # generic domain, ledger, and SQLite authority
│   ├── society-pi/            # generic SDK-host peer and session contracts
│   ├── societyd/              # resident authority and process custody
│   └── societyctl/            # typed local control/query client
├── packages/
│   └── society-pi-host/       # generic pinned TypeScript SDK adapter
...
The application paths above are the searchable XSH contract. A later rename
must update application types, fixtures, judges, diagrams, and the application
glossary together. Generic ownership may not collapse into a generic workflow
framework, and the application adapter must not acquire generic authority.

The circuit's behavior cases and observation values are closed application Rust
types. The generic kernel must eventually bind their sealed results through
named commands; current parsing does not provide that integration. Markdown in
the circuit tree is a prompt, projection, or human explanation, never an
alternate JSON workflow definition.
...
| `48173ed` | closed parsing for the exact C01-C05 negative-control report plus real provider-free controls: C03's task-shaped fake never starts its assigned child and fails both owned-lifecycle and varied-output checks; C05 builds a detached one-line runtime candidate and proves immutable inherited-stderr compatibility rejects it | sealed evaluator, source, build, binary, and report identity; durable experiment/actor/treatment authority; process ownership or lifecycle receipts; evidence admission; and the Milestone-7 end-to-end judge |
...
The current M4 supervisor and M5 bridge remain a bounded native process
foundation. The daemon privately drives one provider-free Office child through
the durable admission, protocol, complete Create delivery, live-child Office,
Dispose, reap, signal/liveness, stream-content, and finalization transitions.
The resident serving loop has no scheduler call site for that bridge, and the
driver does not Prompt, normalize semantic settlement or cost, admit a
transcript file, dispose the workspace, or orchestrate post-restart
containment. A supplied host artifact and package-set digest are verified
bytes; this provider-free foundation does not prove that an arbitrary adapter
...
these commands, or reconcile semantic submissions. The generic Office-session
Dispose foundation now persists `Authorize-before-write -> delivered -> accepted
-> final Known/failure -> Disposed`: it authorizes before the physical pipe
write, records only complete delivery, and requires final Known usage
immediately after acceptance before the next transcript-flush `Disposed`
receipt. The daemon verifies a materialized transcript under owned filesystem
custody and seals its exact bytes through the sole content writer before the
terminal receipt; a no-Prompt session may be materialized and sealed with an
explicitly absent first prompt. Only the lazy missing-file arm is
unmaterialized and has no content object; neither absence arm may invent a
first prompt or content. The Known branch reconciles the existing parent reservation,
releasing its unused reserve or freezing a known overrun. A final accounting
failure freezes the reservation, starts containment, and has no synthetic
`Disposed` successor. Child reap remains separate process custody.

There is still no resident scheduler/control-loop call site, post-restart
recovery, workspace disposal, semantic submission, paid/native qualification,
or XSH end-to-end execution path.
